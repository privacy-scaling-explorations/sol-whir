...
import {WhirConfig, RoundParameters, WhirProof, MerkleProof, Statement} from "../WhirStructs.sol";
...
library JSONUtils {
    function jsonStatementToStatement(JSONStatement memory jsonStatement) external pure returns (Statement memory) {
        ...
        statement.points = new BN254.ScalarField[][](jsonStatement.points.length);
        ...
        for (uint256 i = 0; i < jsonStatement.points.length; i++) {
            BN254.ScalarField[] memory point = new BN254.ScalarField[](jsonStatement.points[i].length);
            for (uint256 j = 0; j < jsonStatement.points[i].length; j++) {
                point[j] = BN254.ScalarField.wrap(uint256(jsonStatement.points[i][j]));
            }
    ...
    function jsonWhirProofToWhirProof(JSONWhirProof memory jsonWhirProof) external pure returns (WhirProof memory) {
        ...
        for (uint256 i = 0; i < jsonWhirProof.merkleProofs.length; i++) {
            ...
            mp.depth = jsonWhirProof.merkleProofs[i].depth;
            mp.decommitments = jsonWhirProof.merkleProofs[i].decommitments;
