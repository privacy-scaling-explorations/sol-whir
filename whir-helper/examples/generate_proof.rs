...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // // Print out the whole leaf level:
    // for i in 0..(1 << 4) {
    //     println!(
    //         "Leaf {}: {:?}",
    //         i,
    //         &hex::encode(merkle_tree.get_leaf_sibling_hash(i ^ 1).as_ref())
    //     );
    // }

    let mut tree_rng = StdRng::from_entropy();
    //let indices_to_prove = vec![3, 15, 13, 4, 0, 9];
    //Let's select ~10 random unique indices to prove
    let indices_to_prove: Vec<usize> = (0..10)
